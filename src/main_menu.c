...
static void Task_NewGameSkipBirch_StartNaming(u8);
static void Task_NewGameSkipBirch_ProcessGender(u8);
...
// Birch intro assets removed: graphics and palettes excluded to save space.
    ...
    if (!gPaletteFade.active)
    {
        ...
        switch (action)
        {
            ...
            default:
                ...
                gTasks[taskId].func = Task_NewGameSkipBirch_StartNaming;
                break;
...
/* Birch intro implementation removed. Gender/naming helpers remain below. */
static void NewGameBirchSpeech_ShowGenderMenu(void)
{
...
// Start naming flow: show gender menu, handle selection, then go to naming screen.
static void Task_NewGameSkipBirch_StartNaming(u8 taskId)
{
    if (!gPaletteFade.active)
    {
        FreeAllWindowBuffers();
        ...
        gTasks[taskId].func = Task_NewGameSkipBirch_ProcessGender;
    }
}

static void Task_NewGameSkipBirch_ProcessGender(u8 taskId)
{
